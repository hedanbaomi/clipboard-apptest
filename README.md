...
- **修复夸克浏览器兼容性** - 夸克占用剪切板锁导致内容无法捕获，新增 `OpenClipboard` 重试机制（最多5次，间隔50ms）、剪切板变化防抖（300ms）、连续失败熔断（10次上限）
- **修复剪切板内容丢失** - 监听线程在读取内容成功后才更新变化计数，避免读取失败时内容永久丢失
- **修复 Ctrl 键失效** - 快捷键注册从 `suppress=True` 改为 `suppress=False`，不再拦截修饰键的按键事件
- **修复开机自启关闭无效** - 设置中关闭自启后同步更新 `config.json`，防止下次启动时被重新开启；操作失败时显示错误提示
- **网页复制无法识别** - 修复从浏览器复制时剪切板同时包含文本和位图格式，优先读取文本而非图片
- **新增日志系统** - 运行日志保存在 `logs/YYYYMMDD.log`，便于问题排查
